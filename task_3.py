...
'Ж': 5, 'З': 5, 'И': 1, 'Й': 4, 'К': 2, 'Л': 2, 'М': 2, 'Н': 1, 'О': 1, 'П': 2, 'Р': 1, 'С': 1, 
'Т': 1, 'У': 2, 'Ф': 10, 'Х': 5, 'Ц': 5, 'Ч': 5, 'Ш': 8, 'Щ': 10, 'Ъ': 10, 'Ы': 4, 'Ь': 3, 'Э': 8, 'Ю': 8, 'Я': 3}

word = input("Введите слово: ").upper()

if all(letter in eng_values for letter in word):
    for letter in word:
        value += eng_values[letter]
elif all(letter in rus_values for letter in word):
    for letter in word:
        value += rus_values[letter]
else:
    print("Ошибка: слово должно содержать только буквы одного языка.")

print("Стоимость слова", word, "равна", value, "очкам.")